        ...
        const std::string & config_name)
{
    ...
    std::string compression_key = config.getBool(config_name + ".enable_compression", false) ? "1" : "0";
    ...
    if (config.has(config_name + ".replica"))
    {
        ...
        for (const auto & replica_config_key : replica_keys)
        {
            ...
            if (replica_config_key.starts_with("replica"))
            {
                ...
                entry_name += (entry_name.empty() ? "" : "|") + tmp_user + "@" + tmp_host + ":" + tmp_port + "/" + db + "?compression=" + compression_key;
            }
        }
    }
    else
    {
        entry_name = user + "@" + host + ":" + port + "/" + db + "?compression=" + compression_key;
    }
    return entry_name;
}
